...
(function getBzConfig() {
  if (!window.bzConfig) {
    const url = `${getLink('source')}/common/js/config.js`;
...
function wechatShare(options) {
  ...
  if (!jsdRequestData) {
    jsonp(apiUrl, data => {
      jsdRequestData = data;
      const wxConfig = setWechatConfig(data, opts);
      wxConfig.init();
    })
  } else {
    const wxConfig = setWechatConfig(jsdRequestData, opts);
    wxConfig.init();
  }
}
